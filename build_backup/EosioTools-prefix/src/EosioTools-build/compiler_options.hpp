...
static cl::opt<std::string> eosio_imports_opt(
    "eosio-imports",
    cl::desc("Set the file for celesos.imports"),
    cl::Hidden,
    cl::cat(LD_CAT));
...
static void GetCompDefaults(std::vector<std::string>& copts) {
   const char* eosio_apply_suff = ".dylib";
...
static Options CreateOptions(bool add_defaults=true) {
   ...
   if (!eosio_imports_opt.empty()) {
      ...
      ldopts.emplace_back("--allow-undefined-file="+eosio::cdt::whereami::where()+"/../celesos.imports");
   }
#endif
...
#endif
   return {output_fn, abigen_inputs.str(), inputs, link, abigen, pp_dir, abigen_output, abigen_contract, copts, ldopts, agopts, agresources};
}
